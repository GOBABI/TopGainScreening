...
def _fetch_gainers_pykrx():
    ...
    try:
        ...
        df['change_pct'] = df[chg_col] if chg_col else (df['종가'] - df['시가']) / df['시가'] * 100
        df = df[df['change_pct'] >= 5].sort_values('change_pct', ascending=False).head(50)
        for ticker_code, row in df.iterrows():
            sym = str(ticker_code) + '.KS'
            try:
...
def fetch_gainers_kr():
    ...
    try:
        ...
        quotes = [q for q in result.get('quotes', []) if str(q.get('symbol','')).endswith('.KS')]
        if quotes:
    ...
    # 3차: Yahoo KR REST
    if not quotes:
        quotes = [q for q in _fetch_gainers_yf_kr() if str(q.get('symbol','')).endswith('.KS')]
...
def fetch_market_kr():
    ...
    for sym, key in [('^KS11','kospi'),('^KQ11','kosdaq'),('^KS200','ks200')]:
        try:
            ...
            out[key] = {'price': round(p,2), 'chg': round((p-p1)/p1*100,2),
                        'week': round((p-float(h['Close'].iloc[0]))/float(h['Close'].iloc[0])*100,2)}
        except Exception as e:
...
def fetch_ticker_detail(ticker):
    ...
    try:
        ...
        data['news'] = [
            ...
            for n in news_raw[:5] if (n.get('content',{}).get('title') or n.get('title'))
        ]
    except Exception:
        data['news'] = []
    return data
...
def calc_adx(high, low, close, p=14):
    import pandas as pd
    hi = high.reset_index(drop=True); lo = low.reset_index(drop=True); cl = close.reset_index(drop=True)
    tr, dp, dm = [], [], []
...
def qullamaggie_position(price, ma200, high52, ytd):
    ...
    if ytd >= 100 and pct52 >= 90:   return 'a', '급등 초입 모멘텀 (30~100%↑ 초기 단계)'
    elif ytd >= 30 and pct52 < 85:   return 'b', '1~3개월 조정 후 재돌파 구간 (선호)'
    elif pct52 >= 90 and above_pct < 20: return 'b', '신고가권 타이트 횡보 (최적진입구간)'
    elif above_pct > 50:             return 'a', '200MA 대비 고점권 — 추격 주의'
    else:                             return 'c', '일반 상승 구간'
...
def analyze(ticker):
    ...
    vol_trend = 'N/A'; vol_contraction = False
    if len(h) >= 20:
        avg20 = float(h['Volume'][-20:].mean()); avg5 = float(h['Volume'][-5:].mean())
        vol_trend = '증가' if avg5 > avg20*1.2 else ('감소' if avg5 < avg20*0.8 else '보합')
        vols = h['Volume'][-20:].values
        q1,q2,q3,q4 = vols[:5].mean(),vols[5:10].mean(),vols[10:15].mean(),vols[15:].mean()
        vol_contraction = bool((q4 < q3 and q3 < q2) or (q4 < q1 * 0.7))
    return {
        '200ma': round(ma200v,2), 'above_200ma': bool(price > ma200v),
        'rsi': round(rsi,1), 'adx': round(adx,1), 'macd_bull': bool(macd_bull),
        '52w_pct': pct52, '52w_high': round(high52,2), 'ytd': ytd, 'adr': adr,
        'ql_pos': ql_pos, 'ql_desc': ql_desc, 'vol_trend': vol_trend, 'vol_contraction': vol_contraction,
    }
...
def score_stock(ta, sector, industry):
    ...
    if any(k in (sector+industry).lower() for k in AI_SECTORS): s += 1
    return s

def auto_risks(ta, detail):
    risks = []
    if ta['rsi'] > 70:  risks.append(f"RSI {ta['rsi']:.0f} — 단기 과열 구간")
    if ta['adr'] > 5:   risks.append(f"ADR {ta['adr']:.1f}% — 변동성 큼")
    sp = detail.get('shortPercentOfFloat') or 0
    if sp and sp > 0.1: risks.append(f"공매도 {sp*100:.1f}%")
    if ta['ql_pos'] == 'a' and ta['ytd'] >= 100: risks.append("YTD 100%↑ 급등주 — 차익실현 매물 주의")
    if not ta['macd_bull']: risks.append("MACD 매수신호 미발생")
    if ta['ql_pos'] == 'b' and not ta.get('vol_contraction'): risks.append("b 구간이나 거래량 수축 미확인")
    beta = detail.get('beta') or 0
    if beta and beta > 2: risks.append(f"Beta {beta:.1f} — 과대 반응 가능")
    if not risks: risks.append("주요 기술적 리스크 없음 — 손절 원칙 준수")
    return risks
...
def korean_desc(detail, ta, q):
    ...
    s_ko = SECTOR_KO.get(sector, sector); i_ko = INDUSTRY_KO.get(industry, industry)
    try:
        rev_g = float(detail.get('revenueGrowth') or 0)
    except (ValueError, TypeError):
        rev_g = 0
    try:
        pe_fwd = float(detail.get('forwardPE') or 0)
    except (ValueError, TypeError):
        pe_fwd = 0
    analysts = detail.get('numberOfAnalystOpinions') or 0
    rec_ko = {'strong_buy':'강력매수','buy':'매수','hold':'보유'}.get(detail.get('recommendationKey',''),'')
    growth_txt = (f'매출 성장률 {rev_g*100:.0f}%로 고성장 중이며' if rev_g > 0.3
                  else f'매출 성장률 {rev_g*100:.0f}%의 안정적 성장세를 유지하며' if rev_g > 0
                  else f'매출이 {abs(rev_g)*100:.0f}% 감소하는 역성장 구간에 있으며' if rev_g < 0 else '')
    val_txt = (''
               if not pe_fwd or pe_fwd <= 0 else
               f'선행 P/E {pe_fwd:.0f}배로 저평가' if pe_fwd < 15 else
               f'선행 P/E {pe_fwd:.0f}배의 합리적 밸류에이션' if pe_fwd < 35 else
               f'선행 P/E {pe_fwd:.0f}배로 고성장 프리미엄 반영')
    tech_txt = ('52주 신고가권에서 강한 모멘텀을 보이고 있습니다' if ta['52w_pct'] >= 90
                else f'연초 대비 {ta["ytd"]:+.0f}%의 강한 YTD 모멘텀을 보유하고 있습니다' if ta['ytd'] >= 50
                else '200MA 위에서 상승 추세를 유지하고 있습니다')
    analyst_txt = f'애널리스트 {analysts}명 컨센서스 {rec_ko}.' if analysts and rec_ko else ''
    return (f"{name}은(는) {s_ko} / {i_ko} 분야 기업입니다. "
            f"{growth_txt+' ' if growth_txt else ''}{val_txt+'. ' if val_txt else ''}"
            f"{tech_txt}. {analyst_txt}").strip()
...
def run_screening_kr(gainers):
    def _vol_ok(q):
        ...
        return q['regularMarketVolume'] >= (avg * 1.5 if avg else 50_000)
    ...
    for q in p1:
        ...
        passed.append({
            'ticker': ticker, 'exchange': q.get('exchange', 'KRX'),
            'name': q.get('shortName') or '', 'full_name': detail.get('longName') or q.get('shortName') or '',
            'price': round(q['regularMarketPrice'], 0), 'change_pct': round(q['regularMarketChangePercent'], 2),
            'volume': q['regularMarketVolume'], 'avg_vol': avg_vol,
            'vol_ratio': round(q['regularMarketVolume'] / avg_vol, 1),
            'market_cap': q['marketCap'], 'sector': sector, 'industry': industry,
...
def watch_status(days, appearances, ql_pos):
    if days == 0:       return "신규 등장", "orange"
    elif days <= 2:     return f"{days}일차 — 베이스 형성 대기", "gold"
    elif days <= 7:     return (f"{days}일차 — 진입 가능 (b 선호)" if ql_pos == 'b' else f"{days}일차 — 진입 가능"), "green"
    elif days <= 15:    return f"{days}일차 — 최적진입구간 트리거 대기", "teal"
    else:               return f"{days}일차 — 재평가 필요", "red"
...
def _is_reentry(last_seen_str):
    try:
        ...
        gap = 0; cur = last + timedelta(days=1)
        while cur <= today:
            if cur.weekday() < 5: gap += 1
...
def update_watchlist(passed):
    ...
    for s in passed:
        tk = s['ticker']
        snap = {'last_rsi': round(s.get('rsi',0),1), 'last_adx': round(s.get('adx',0),1),
                'last_macd_bull': bool(s.get('macd_bull',False)),
                'last_52w_pct': round(s.get('52w_pct',0),1), 'last_ytd': round(s.get('ytd',0),1)}
        if tk in tickers:
            e = tickers[tk]
            if _is_reentry(e.get('last_seen', TODAY)):
                log(f"  재진입 [{tk}]"); e['first_seen'] = TODAY; e['appearances'] = 1
            elif e.get('last_seen') != TODAY:
                e['appearances'] = e.get('appearances', 1) + 1
            e.update({'last_seen': TODAY, 'last_price': s['price'], 'last_change_pct': s['change_pct'],
                      'last_score': s['score'], 'last_ql_pos': s['ql_pos'], 'last_ql_desc': s['ql_desc'],
                      'name': s['name'], 'sector': s['sector'], 'industry': s.get('industry',''),
                      'exchange': s.get('exchange', e.get('exchange','KRX')), **snap})
        else:
            tickers[tk] = {'first_seen': TODAY, 'last_seen': TODAY, 'appearances': 1,
                           ...
                           'exchange': s.get('exchange','KRX'), **snap}
    wl['tickers'] = tickers
    save_watchlist(wl)
...
def refresh_watchlist_ta(wl, today_tickers):
    ...
    for tk in stale:
        ...
        try:
            ...
            pass
        e.update({'last_rsi': round(ta['rsi'],1), 'last_adx': round(ta['adx'],1),
                  ...
                  'last_score': score_stock(ta, e.get('sector',''), e.get('industry','')),
                  'last_seen': TODAY})
    save_watchlist(wl)
    return wl
...
def dump_json(passed, mkt, wl=None):
    compact = {
        ...
        'passed': [{'ticker': s['ticker'], 'name': s['name'], 'sector': s['sector'],
                    ...
                    'summary': (s['summary'] or '')[:200]} for s in passed],
        'passed_count': len(passed),
        'watchlist': {
            tk: {'first_seen': e.get('first_seen'),
                 ...
                 'status': watch_status(days_since_date(e.get('first_seen', TODAY)),
                                        e.get('appearances',1), e.get('last_ql_pos',''))[0]}
            for tk, e in (wl.get('tickers',{}) if wl else {}).items()
        },
    }
    with open(JSON_OUT, 'w', encoding='utf-8') as f: json.dump(compact, f, ensure_ascii=False, indent=2)
    log(f"JSON 덤프: {JSON_OUT}")

def build_narrative(passed, mkt):
    kospi = mkt.get('kospi', {}); kosdaq = mkt.get('kosdaq', {})
    kc = kospi.get('chg',0); dc = kosdaq.get('chg',0)
    status = "약세" if kc<-0.5 and dc<-0.5 else "강세" if kc>0.5 and dc>0.5 else "혼조"
    lines = [f"📝 한국 시장 스크리닝 코멘트 [{TODAY}]",
             "━"*18, "", "📰 시장 코멘트",
             f"코스피 {kc:+.2f}% / 코스닥 {dc:+.2f}%로 {status} 마감했습니다.",
             "", "━"*18]
    if not passed:
        lines.append("❌ 오늘은 조건에 맞는 KOSPI 종목이 없었습니다.")
    else:
        lines += [f"🔍 통과 종목 분석 (총 {len(passed)}개)", ""]
        for i, s in enumerate(passed, 1):
            cat = s['catalysts'][0] if s['catalysts'] else "기술적 모멘텀 중심"
            risk = s['risks'][0] if s['risks'] else "손절 원칙 준수"
            lines += [f"#{i} {s['ticker']} — {s['name']}",
                      f"• 오늘 +{s['change_pct']:.1f}% 상승. {cat[:60]}",
                      f"• 모멘텀 위치: {s['ql_pos']} — {s['ql_desc']}",
                      f"• 체크: RSI {s['rsi']:.0f} / ADX {s['adx']:.0f} / 점수 {s['score']}/9",
                      f"• 주의: {risk[:60]}", ""]
    lines += ["━"*18, "⚠️ 투자 책임은 본인에게 있습니다"]
    return "\n".join(lines)
...
def send_telegram_narrative(text):
    ...
    LIMIT = 4000
    if len(text) <= LIMIT:
        _send_one_message(text); return
    chunks, buf = [], ""
    for line in text.split("\n"):
        candidate = buf + line + "\n"
        if len(candidate) > LIMIT:
            if buf: chunks.append(buf.rstrip("\n"))
            buf = line + "\n"
        else:
            buf = candidate
    if buf: chunks.append(buf.rstrip("\n"))
    for i, chunk in enumerate(chunks, 1):
        ...
        if i < len(chunks): time.sleep(1)

def load_archive_dates():
    try:
        content = _gh_read_data("archive_kr.json")
        if content:
            with open(ARCHIVE_PATH_KR, 'w') as f: f.write(content)
    except Exception:
        pass
    if os.path.exists(ARCHIVE_PATH_KR):
        with open(ARCHIVE_PATH_KR, 'r') as f: return json.load(f).get('dates', [])
    return []

def save_archive_dates(dates):
    with open(ARCHIVE_PATH_KR, 'w') as f: json.dump({'dates': dates}, f)
    try:
...
def github_pages_deploy_kr(archive_dates):
    ...
    if not os.path.exists(HTML_OUT_KR):
        log("KR HTML 파일 없음 — GitHub Pages 배포 스킵"); return
    try:
        with open(HTML_OUT_KR, 'rb') as f: html_b64 = base64.b64encode(f.read()).decode()
        base_url = f"https://api.github.com/repos/{GITHUB_REPO}"
        def _upsert(path, c):
            url = f"{base_url}/contents/{path}"
            r = requests.get(url, headers=GH_HEADERS, params={"ref": "gh-pages"}, timeout=15)
            sha = r.json().get("sha") if r.ok else None
            pl = {"message": f"deploy kr: {TODAY}", "content": c, "branch": "gh-pages"}
            if sha: pl["sha"] = sha
            return requests.put(url, headers=GH_HEADERS, json=pl, timeout=60).ok
        ok1 = _upsert("kr_index.html", html_b64)
        ok2 = _upsert(f"kr_{TODAY}.html", html_b64)
        log(f"KR GitHub Pages 배포 {'완료' if ok1 and ok2 else '일부 실패'}")
    except Exception as e:
