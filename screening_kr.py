...
한국 주식(KOSPI/KOSDAQ) 상위 급등주 스크리닝
수동 실행 전용 — 자동 스케줄 없음
"""
...
def _fetch_gainers_pykrx():
    """pykrx로 KOSPI/KOSDAQ 당일 상승률 상위 50개 반환"""
    try:
...
def fetch_gainers_kr():
    ...
    # 1차: yf.screen region=KR
    quotes = []
    try:
        result = yf.screen("day_gainers", count=50)
        raw = result.get('quotes', [])
        # KR 종목만 필터 (.KS / .KQ suffix)
        quotes = [q for q in raw if str(q.get('symbol','')).endswith(('.KS', '.KQ'))]
        if quotes:
            log(f"  yf.screen KR 결과: {len(quotes)}개")
    except Exception as e:
        log(f"  yf.screen 실패({e})")

    # 2차: Yahoo Finance REST KR
    if not quotes:
        raw2 = _fetch_gainers_yf_kr()
        quotes = [q for q in raw2 if str(q.get('symbol','')).endswith(('.KS', '.KQ'))]
    ...
    if not quotes and _check_pykrx():
        ...
        return quotes  # pykrx는 이미 표준 dict 형태로 반환
...
# ── 스크리닝 메인 로직 ────────────────────────────────────────────────
# 한국장 기준: +5% 이상, 1,000원 이상, 거래량 1.5x, 시총 50조 KRW 이하
